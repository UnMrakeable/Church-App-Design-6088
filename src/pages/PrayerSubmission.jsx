...
const PrayerSubmission = () => {
  ...
  const { addPrayer, addTranslation, prayers } = usePrayers();
  ...
  const categories = ['English', 'Tagalog', 'Ilocano', 'Spanish', 'French'];
  ...
  const handleCaptchaVerification = () => {
    ...
    const userInput = prompt("Please enter the sum of 6 + 4 to verify you're human:");
    if (userInput === "10") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.isTranslation && formData.originalPrayerId) {
        // Add as translation
        ...
        addTranslation(formData.originalPrayerId, translationData);

        // Also add the prayer itself with reference to original
        addPrayer({
          ...prayerData,
          originalPrayerId: formData.originalPrayerId,
          isTranslation: true
        });
      } else {
      ...
      alert('There was an error submitting your prayer. Please try again.');
    } finally {
    ...
    >
      ...
      <motion.div variants={itemVariants} className="text-center">
        ...
        <p className="text-peace-600 dark:text-peace-300 font-body">
          Share prayers with our worship community. {!isAdminOrEditor && 'All submissions will be reviewed before publishing.'}
        </p>
      </motion.div>

      {/* Notice - Only for non-admin users */}
      {!isAdminOrEditor && (
        <motion.div variants={itemVariants} className="bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-xl p-4">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-blue-700 dark:text-blue-300 text-sm font-body">
                All submitted prayers will be reviewed by our administrators to ensure appropriateness and quality. You'll be notified once your submission has been approved and published.
              </p>
            </div>
          </div>
        </motion.div>
      ...
      >
        ...
        <div className="border-b border-peace-200 dark:border-peace-700 pb-6">
          <div className="flex items-center space-x-3 mb-4 p-4 bg-peace-50 dark:bg-peace-700 rounded-xl">
            ...
            <label htmlFor="isTranslation" className="text-base md:text-lg font-medium text-peace-700 dark:text-peace-300 font-body flex items-center space-x-2">
              ...
              <span>This is a translation of an existing prayer</span>
            </label>
          </div>
          ...
          {formData.isTranslation && (
            <div>
              ...
              <select
                name="originalPrayerId"
                value={formData.originalPrayerId}
                onChange={handleInputChange}
                required
                className="w-full px-4 py-3 bg-peace-100 dark:bg-peace-700 rounded-xl text-peace-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 font-body"
              >
                <option value="">Select original prayer...</option>
                {getPrayersInSameLanguage(formData.category).map(prayer => (
        ...
        <div className="border-b border-peace-200 dark:border-peace-700 pb-6">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-4 py-3 bg-peace-100 dark:bg-peace-700 rounded-xl text-peace-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 font-body"
              placeholder="e.g., opening, worship, fellowship, thanksgiving"
            />
          </div>
        </div>
        ...
        <div className="border-b border-peace-200 dark:border-peace-700 pb-6">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Enter the complete prayer text here..."
            />
            <p className="text-xs text-peace-500 dark:text-peace-400 mt-1 font-body">
              Enter the full prayer text. Use line breaks to separate different parts of the prayer.
            </p>
          </div>
        </div>
        ...
        <div className="flex space-x-4 pt-4">
          ...
          <button
            ...
            className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-xl hover:from-purple-600 hover:to-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-body flex items-center justify-center space-x-2"
          >
